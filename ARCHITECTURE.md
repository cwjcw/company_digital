...
- `apps/api/src/modules/notifications`: KDOS 通知后端基础设施与消息中心管理 API。通知规则、`notification_outbox` 与逐接收人 `notification_delivery_logs` 使用 PostgreSQL 事务、租户 RLS 和幂等 dedup key；Dispatcher 领取前在 API 内完成规则匹配和动态责任人解析，领取使用 `FOR UPDATE SKIP LOCKED`。消息中心只显示服务端注册事件、受支持的接收人和企业微信工作通知，规则/日志/失败重试按系统管理员或资源所属模块管理员授权；不在业务表工具栏增加推送按钮。
