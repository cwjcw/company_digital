...
SELECT COALESCE(c.employee_no,c.wechat_user_id),c.name,'$2b$12$MtmiHvPQoO81c8XiUaKbn.vOb6YqHIJDcvqYZ.AkLytztdGVNWZky',c.enabled,NULL,c.employee_no,c.wechat_user_id,c.position,c.department_paths,true,NULL
...
WHERE c.rn=1 AND NOT EXISTS (SELECT 1 FROM users u WHERE u.username=COALESCE(c.employee_no,c.wechat_user_id) OR (c.employee_no IS NOT NULL AND u.employee_no=c.employee_no));
