...
export class PlanningOperationsApplicationService {
  ...
  async weeklyPeriods(actor: PlanningActor) { this.assert(actor,"weekly-plan","read"); return this.repository.listWeeklyPeriods(await this.tenant(actor)); }
  ...
  async syncWeekly(periodId: string, actor: PlanningActor) { this.assert(actor,"weekly-plan","import"); return this.repository.syncWeeklyItems(await this.tenant(actor),periodId,actor); }
