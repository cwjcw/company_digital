...
export class MarketingController {
  ...
  @Get("business-customer-mappings/export") async exportMappings(@Query() query: Record<string,string|undefined>, @Req() req: MarketingRequest, @Headers("x-tenant-code") tenant: string | undefined, @Ip() ip: string, @Res() response: Response) {
    /* KN-FILTER-001：导出必须与列表共用 search + FilterGroup，禁止页面用 typed、导出用 legacy。 */
    const rows: any[] = await this.application.exportMappings({ search: query.search, filterGroup: query.filterGroup, sortField: query.sortField, sortOrder: query.sortOrder }, this.actor(req, tenant, ip)) as any[];
  ...
  @Get("order-schedules") listSchedules(@Query() query: Record<string,string|undefined>, @Req() req: MarketingRequest, @Headers("x-tenant-code") tenant: string | undefined, @Ip() ip: string) { if(!query.page&&!query.pageSize)return this.application.listSchedules(query.search,this.actor(req,tenant,ip));let filters={};try{filters=JSON.parse(query.filters??"{}");}catch{filters={};}return this.application.listSchedulesPage({...query,page:Number(query.page),pageSize:Number(query.pageSize),filters,filterGroup:query.filterGroup},this.actor(req,tenant,ip)); }
  @Get("order-schedules/export") async exportSchedules(@Query() query: Record<string,string|undefined>, @Req() req: MarketingRequest, @Headers("x-tenant-code") tenant: string | undefined, @Ip() ip: string, @Res() response: Response) {
    /* KN-FILTER-001：导出与列表共用同一 WHERE，保证跨页导出与筛选一致。 */
    const rows: any[] = await this.application.exportSchedules({ search: query.search, filterGroup: query.filterGroup, sortField: query.sortField, sortOrder: query.sortOrder, completion: query.completion, dueStart: query.dueStart, dueEnd: query.dueEnd, exactOrderNumber: query.exactOrderNumber }, this.actor(req, tenant, ip)) as any[];
