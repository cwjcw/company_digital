  ...
  async preview(versionId: string, file: Express.Multer.File, actor: PlanningActor) {
    ...
    sourceRows.forEach((source, index) => {
      ...
      const input: CreatePlanItemInput = {
        ...
        historicalInboundQuantity: numberValue(mapped.get("historicalInboundQuantity")),
        currentInboundQuantity: numberValue(mapped.get("todayInboundQuantity")),
        unitPrice: numberValue(mapped.get("unitPrice")),
        deliveryDate: dateValue(mapped.get("customerDueDate")),
        exception: String(mapped.get("orderException") ?? "").trim() || undefined,
