...
type Action = "RESUBMIT" | "REQUESTER_APPROVE" | "REQUESTER_REJECT" | "ASSIGN" | "SUBMIT_PLAN" | "HANDLER_APPROVE" | "HANDLER_REJECT";
...
type RequestRow = {
  id: string; requestNumber: string; title: string; category: string; description: string; businessValue?: string | null;
  urgency: string; desiredDate?: string | null; status: string; requesterId: string; requesterName: string;
  requesterManagerId: string; requesterManagerName: string; handlerId?: string | null; handlerName?: string | null;
  ...
  availableActions: Action[]; events?: Array<{ id: string; action: string; actorName: string; comment?: string | null; toStatus: string; createdAt: string }>;
};

const statusMeta: Record<string, { label: string; color: string; step: number }> = {
  PENDING_REQUESTER_APPROVAL: { label: "待填写人上级审批", color: "gold", step: 0 },
  REQUESTER_REJECTED: { label: "填写人上级已驳回", color: "red", step: 0 },
  PENDING_ADMIN_ASSIGNMENT: { label: "待管理员分配", color: "blue", step: 1 },
  PENDING_HANDLER_PLAN: { label: "待处理人评估", color: "cyan", step: 2 },
  PENDING_HANDLER_MANAGER_APPROVAL: { label: "待处理人上级审批", color: "purple", step: 3 },
  HANDLER_MANAGER_REJECTED: { label: "处理人上级已驳回", color: "red", step: 2 },
  APPROVED_FOR_DEVELOPMENT: { label: "已批准开发", color: "green", step: 4 }
};
const actionLabels: Record<Action, string> = {
  RESUBMIT: "修改并重提", REQUESTER_APPROVE: "上级通过", REQUESTER_REJECT: "上级驳回",
  ASSIGN: "分配处理人", SUBMIT_PLAN: "填写资源与工期", HANDLER_APPROVE: "开发审批通过", HANDLER_REJECT: "开发审批驳回"
};
const eventLabels: Record<string, string> = {
  SUBMIT: "填写人提交需求", RESUBMIT: "填写人修改后重提", REQUESTER_APPROVE: "填写人上级审批通过",
  REQUESTER_REJECT: "填写人上级驳回", ASSIGN: "管理员分配处理人员", SUBMIT_PLAN: "处理人提交资源与工期",
  HANDLER_APPROVE: "处理人上级审批通过", HANDLER_REJECT: "处理人上级驳回"
};
...
export function DevelopmentRequestsPage() {
  ...
  const [resubmitting, setResubmitting] = useState<RequestRow>();
  const [detailId, setDetailId] = useState<string>();
  const [decision, setDecision] = useState<{ row: RequestRow; action: Action }>();
  ...
  const requests = useQuery({
    queryKey: ["development-requests", scope, search],
    queryFn: () => api<RequestRow[]>(`/development-requests?scope=${scope}&search=${encodeURIComponent(search)}`)
  });
  ...
  const submit = async (path: string, body: unknown, success: string, method = "POST") => {
    ...
    try {
      ...
      message.success(success); setDecision(undefined); setAssigning(undefined); setPlanning(undefined); setRequestOpen(false); setResubmitting(undefined);
      decisionForm.resetFields(); assignForm.resetFields(); planForm.resetFields(); requestForm.resetFields(); await refresh();
    } catch (error) { message.error((error as Error).message); }
    finally { setSaving(false); }
  };
  const openNew = () => {
    const me = (people.data ?? []).find((person) => person.id === currentUser.sub);
    setResubmitting(undefined); requestForm.resetFields();
    requestForm.setFieldsValue({ category: "系统功能", urgency: "NORMAL", requesterManagerId: me?.managerIds?.[0] }); setRequestOpen(true);
  };
  const openResubmit = (row: RequestRow) => {
    setResubmitting(row); requestForm.setFieldsValue({ ...row, desiredDate: row.desiredDate ? dayjs(row.desiredDate) : null }); setRequestOpen(true);
  };
  const openAssignment = (row: RequestRow) => { setAssigning(row); assignForm.resetFields(); };
  ...
  const startAction = (row: RequestRow, action: Action) => {
    if (action === "RESUBMIT") return openResubmit(row);
  ...
  const counters = useMemo(() => ({
    total: rows.length,
    todo: rows.filter((row) => row.availableActions.length).length,
  ...
  const columns = [
    ...
    { title: "需求标题", dataIndex: "title", width: 240, ellipsis: true },
    ...
    { title: "操作", key: "actions", fixed: "right" as const, width: 260, render: (_: unknown, row: RequestRow) => <Space size={4} wrap>
      <Button size="small" onClick={() => setDetailId(row.id)}>详情</Button>
      {row.availableActions.map((action) => <Button key={action} size="small" type={["REQUESTER_APPROVE", "HANDLER_APPROVE", "ASSIGN", "SUBMIT_PLAN"].includes(action) ? "primary" : "default"} danger={["REQUESTER_REJECT", "HANDLER_REJECT"].includes(action)} onClick={() => startAction(row, action)}>{actionLabels[action]}</Button>)}
    </Space> }
  ];
  ...
  return <div className="development-page">
    ...
    <Card className="development-workbench" bordered={false}>
      ...
      <Table style={{ marginTop: 14 }} rowKey="id" dataSource={rows} loading={requests.isLoading} columns={columns} pagination={{ pageSize: 20, showSizeChanger: true, showTotal: (total) => `共 ${total} 条` }} scroll={{ x: 1850, y: "calc(100vh - 410px)" }} />
    </Card>

    <Modal title={resubmitting ? `修改并重新提交 · ${resubmitting.requestNumber}` : "提报新需求"} width={720} open={requestOpen} confirmLoading={saving} okText={resubmitting ? "重新提交审批" : "提交审批"} onCancel={() => { setRequestOpen(false); setResubmitting(undefined); }} onOk={() => requestForm.validateFields().then((values) => submit(resubmitting ? `/development-requests/${resubmitting.id}/resubmit` : "/development-requests", { ...values, desiredDate: values.desiredDate?.format("YYYY-MM-DD") }, resubmitting ? "需求已重新提交" : "需求已提交给上级领导审批", resubmitting ? "PATCH" : "POST"))}>
      <Alert type="info" showIcon message="提交后先由填写人的上级领导审批；审批通过后由管理员分配处理人员。" style={{ marginBottom: 16 }} />
      <Form form={requestForm} layout="vertical">
        ...
        <Form.Item name="requesterManagerId" label="填写人的上级审批领导" rules={[{ required: true, message: "请选择上级审批领导" }]}><Select showSearch optionFilterProp="label" options={personOptions.filter((option) => option.value !== currentUser.sub)} placeholder="已根据通讯录自动推荐；也可搜索选择" /></Form.Item>
    ...
    <Modal title={decision ? `${actionLabels[decision.action]} · ${decision.row.requestNumber}` : "审批"} open={Boolean(decision)} confirmLoading={saving} okButtonProps={{ danger: decision ? ["REQUESTER_REJECT", "HANDLER_REJECT"].includes(decision.action) : false }} okText={decision ? actionLabels[decision.action] : "提交"} onCancel={() => setDecision(undefined)} onOk={() => decisionForm.validateFields().then((values) => {
      if (!decision) return;
      const requester = decision.action.startsWith("REQUESTER_"); const approved = decision.action.endsWith("APPROVE");
      return submit(`/development-requests/${decision.row.id}/${requester ? "requester-decision" : "handler-manager-decision"}`, { approved, comment: values.comment }, approved ? "审批已通过" : "已驳回并退回修改");
    })}>
      <Paragraph>{decision?.row.title}</Paragraph>
      <Form form={decisionForm} layout="vertical"><Form.Item name="comment" label="审批意见" rules={decision && ["REQUESTER_REJECT", "HANDLER_REJECT"].includes(decision.action) ? [{ required: true, whitespace: true, message: "驳回时必须填写意见" }] : []}><Input.TextArea rows={4} maxLength={2000} placeholder="通过时可选填，驳回时必须说明原因" /></Form.Item></Form>
    </Modal>
    ...
    <Drawer title={detailRow ? `${detailRow.requestNumber} · ${detailRow.title}` : "需求详情"} width={760} open={Boolean(detailId)} onClose={() => setDetailId(undefined)}>
      {detail.isLoading ? <Alert type="info" showIcon message="正在加载需求详情" /> : !detailRow ? <Empty /> : <Space direction="vertical" size={18} style={{ width: "100%" }}>
        <Steps current={currentStep} status={detailRow.status.includes("REJECTED") ? "error" : "process"} size="small" items={[{ title: "填写人上级审批" }, { title: "管理员分配" }, { title: "资源工期评估" }, { title: "处理人上级审批" }, { title: "待开发" }]} />
        ...
        <Card size="small" title="需求说明"><Paragraph style={{ whiteSpace: "pre-wrap", marginBottom: 0 }}>{detailRow.description}</Paragraph></Card>
