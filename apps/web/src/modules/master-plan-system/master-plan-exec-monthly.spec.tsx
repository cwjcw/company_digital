...
describe("KN-MPS-EXEC-001 月计划生产进度口径", () => {
  it("样例：required=500、两个 weekly planned 合计 300、actual 合计 255 → 51%（不是 85%、不是 90%）", async () => {
    ...
    render(<MasterPlanProgressCell process={cutting} resource="mps-monthly-plans" value={0.51} row={{
      requiredQuantity: 500, plannedQuantity: 300, dispatchedWeeklyQuantity: 300, cuttingReportedQuantity: 255
    }} />);
    ...
    /* Hover 分母是月度总需求 500，不是已下达周计划数量 300；顺序：需求 → 已下达 → 累计报工 → 进度。 */
    ...
    /* KN-MPS-UI-001：报工次数彻底移除。 */
    expect(screen.queryByText(/报工次数/)).not.toBeInTheDocument();
  ...
  it("永久锁定：required=500、两个 weekly 全部报满 300 → 60%，绝不能显示 100% 或绿色", async () => {
    render(<MasterPlanProgressCell process={cutting} resource="mps-monthly-plans" value={0.6} row={{
      requiredQuantity: 500, plannedQuantity: 300, dispatchedWeeklyQuantity: 300, cuttingReportedQuantity: 300
    }} />);
  ...
  it("月计划 required=500、actual=500 → 100% 且绿色（整个月度总需求完成）", () => {
    render(<MasterPlanProgressCell process={cutting} resource="mps-monthly-plans" value={1} row={{
      requiredQuantity: 500, plannedQuantity: 300, dispatchedWeeklyQuantity: 300, cuttingReportedQuantity: 500
    }} />);
  ...
  it("周计划语义不变：分母是需求数量 plannedQuantity，Hover 不显示「月度总需求」或「已下达周计划数量」", async () => {
    render(<MasterPlanProgressCell process={cutting} resource="mps-weekly-plans" value={1.05} row={{ plannedQuantity: 100, cuttingReportedQuantity: 105 }} />);
