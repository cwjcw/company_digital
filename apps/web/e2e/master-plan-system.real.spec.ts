...
test.describe("主计划系统真实业务页面", () => {
  ...
  test("PMC通过页面新增出货计划并补齐基础计划后下推周计划", async ({ page }) => {
    ...
    await textarea("品号").fill(item);
    await textarea("品名").fill("测试缺周期页面品项");
    await dialog.locator(".ant-form-item", { hasText: "交次" }).locator("input").fill("1");
    ...
    await dialog.locator(".ant-form-item", { hasText: "产品属性" }).locator("textarea").fill("TEST_重点产品");
    await dialog.locator(".ant-form-item", { hasText: "表面性质" }).locator("textarea").fill("TEST_喷涂");
