...
按事业部统计设备总台账中的监测设备和已指定责任人的设备数量，并结合 `training_status.txt` 生成 BI 风格图片。统计范围固定为事业一部、事业二部、事业三部、事业四部；研发中心不在本次通报范围内。
...
SSH 主机可使用 N8N 容器所见的宿主机网关 `172.19.0.1`，用户为 `Jerry`；认证信息保存在 N8N 的 SSH Credential 中，不得写入命令。节点成功时输出 JSON，其中应包含 `"sent": true`、`"errcode": 0`、`"recipientCount": 11`；命令非零退出时让工作流直接失败，不要继续执行成功分支。
