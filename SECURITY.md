...
- KDOS 通知的 `notification_rules`、`notification_outbox`、`notification_delivery_logs` 均带 `tenant_id`、RLS policy 和显式租户条件；outbox 使用 `tenant_id + dedup_key` 幂等唯一约束，内部 API 以专用 token、tenant 和 worker header 保护，领取和状态回写必须校验 worker 与租户。消息中心 API 另行使用浏览器 `AuthGuard`，服务端按系统管理员/资源所属模块管理员授权，普通用户即使直接调用 URL 也会被拒绝。规则事件、资源、接收人、渠道和模板变量均由后端注册表校验。规则不匹配的事件被隔离为不可重试状态，责任人缺少 `wechat_user_id` 时只记录 `SKIPPED_MISSING_WECHAT_ID`，禁用用户记录 `SKIPPED_DISABLED`，不阻塞其他责任人。企业微信 adapter 不访问数据库、不解析业务接收人，secret 只允许由 `basic_code` 自己的环境加载；验证阶段必须通过环境门禁只允许崔玮杰，未通过门禁的接收人不会调用企业微信。
