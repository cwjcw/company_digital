...
主计划系统各资源的受控 Excel 导入统一使用 `GET /api/v1/master-plan-system/resources/:resource/export` 导出当前查询范围，再通过 `POST .../:resource/import-preview` 和 `POST .../:resource/import-confirm` 两阶段导入；`GET .../:resource/import-template` 用于下载新增模板和填写说明。记录ID是系统生成的稳定唯一标识，版本是防止多人覆盖的乐观锁整数：记录ID和版本同时留空表示新增，系统自动生成二者；二者同时填写表示更新，必须原样保留导出值；只填写其中一个时拒绝导入。新增行同时要求表级 import 与 create 权限，更新行要求 import 权限并重新校验字段权限、数据范围和乐观版本。确认令牌绑定用户、租户、资源、文件内容并有有效期，新增和更新可在同一文件中混合，整批在一个事务中提交并记录逐行及批次审计。同一新增业务键重复时拒绝导入，避免把应更新的数据误建为重复记录。所有入口只接受 xlsx，解析前调用统一加密容器检测，检测到加密文件时原样返回“该文件被加密,请解密后再导入.”。

出货计划手工新增要求品项名称、承接事业部和新旧款必填。事业部基础计划要求最迟评审交期、产品属性、新旧款、表面性质和生产方式必填；产品属性固定为五金、木作、亚克力、五金+木作，表面性质固定为烤漆、电镀。网页、Excel 导入和直接 API 写入均由后端执行相同的必填和选项校验。手工新增或编辑弹窗的确认操作会显示加载状态；客户端校验或服务端拒绝时，弹窗内持续显示具体原因并保留草稿，不再静默无响应。

出货计划、事业部基础计划、事业部周计划、周计划工序明细、客户事业部映射、订单分配和工序周期均允许具备资源 `create` 权限的用户网页手工新增；无该权限时前端不显示新增按钮，后端也拒绝直接调用。基础计划和周计划的上游来源引用允许为空，以区分手工记录；后续命中同一“订单编号 + 品项编码 + 交期编码”时可由投影同步安全关联上游。主计划字段统一使用“品项编码、品项名称、交期编码”。旧字段 `model_age` 的业务含义是“新旧款”（新/旧），不是模具年龄；基础计划不再包含准入状态和准入说明，事业部周计划不再包含计划页数。

周计划工序明细的“所属事业部周计划”用于关联具体的事业部周计划记录，网页按“订单编号 / 品项编码 / 品项名称 / 交期编码”选择和显示，不要求用户识别 UUID。工序顺序是内部排程数据，不再作为业务字段展示。报工日期使用 `YYYY-MM-DD`，手工新增时默认 Asia/Shanghai 当天；当日报工按该日期汇总工序报工表数据，工序状态仍按累计报工与计划数量计算，避免把单日数量误当累计完成量。
...
`POST /api/v1/internal/notifications/claim` and the per-delivery success/failure callbacks are host-only endpoints protected by `KDOS_NOTIFICATION_INTERNAL_TOKEN`, `X-KDOS-Tenant-Id` and `X-KDOS-Worker-Id`; they do not use browser authentication or expose a notification center. The host project `automation/wechat_push_projects/kdos-notification-dispatcher` only calls this API and the default `WeChatPusher` from `/data/automation/code/work/basci/basic_code`; it never connects to PostgreSQL or resolves recipients. TEST MODE is currently forced on in the API: real business recipients are resolved and logged, then the actual recipient is overridden to enabled user CuiWeiJie/崔玮杰. Multiple business recipients produce one actual test send and one callback carrying all delivery IDs. The Dispatcher gate validates that actual target; it does not require Cui WeiJie to be an equipment responsible.

The authenticated message-center API is under `/api/v1/notifications` and is used by `/system/notifications`. It provides registered events/template variables, tenant-scoped rule CRUD, real delivery logs/failures, authorized retry, and a test enqueue endpoint. It does not expose internal claim/success/failure APIs, arbitrary event types, arbitrary recipient parsing, or new delivery channels. The current registered event is `equipment.status.fault_changed` for `equipment-status-report`; supported recipient rules are `EQUIPMENT_RESPONSIBLE` and `FIXED_USERS`, with fixed rules storing only enabled KDOS `users.id` values and resolving WeChat IDs at dispatch time. The event triggers only when fault duration changes and the new value is greater than zero. Test requests remain subject to the Cui WeiJie actual-recipient safety gate.
